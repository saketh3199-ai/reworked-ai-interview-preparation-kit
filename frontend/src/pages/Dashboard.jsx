...
import { useGetKitsQuery } from "../services/kitApi";
                ...
                <div>
                    ...
                    <nav className="mt-10 hidden flex-col gap-1 lg:flex">
                        {NAV_ITEMS.map((item, i) => (
                            ...
                            >
                                <span className="font-mono text-[10px] text-stone-600">0{i + 1}</span>
                                {item.label}
                            </button>
                        ))}
                    </nav>
            ...
            <main className="flex-1 px-6 py-10 sm:px-10 lg:px-14 lg:py-14">
                ...
                <div className="mt-10 grid grid-cols-2 gap-4 lg:grid-cols-4">
                    <div className="col-span-2 rounded-2xl bg-stone-950 p-6 text-stone-50 sm:col-span-1">
                        <p className="font-mono text-[11px] uppercase tracking-wide text-stone-500">Total kits</p>
                    ...
                    <div className="rounded-2xl border border-stone-200 bg-white p-6">
                        <p className="font-mono text-[11px] uppercase tracking-wide text-stone-400">Status</p>
                    ...
                    <div className="rounded-2xl border border-stone-200 bg-white p-6">
                        <p className="font-mono text-[11px] uppercase tracking-wide text-stone-400">Questions</p>
                        <p className="mt-3 font-serif text-2xl text-stone-900">
                            {kits.reduce((sum, k) => sum + (k.kit?.questions?.length || 0), 0)}
                        </p>
                    </div>

                    <div className="rounded-2xl border border-stone-200 bg-white p-6">
                        <p className="font-mono text-[11px] uppercase tracking-wide text-stone-400">Account</p>
                ...
                <div className="mt-12">
                    <div className="mb-5 flex items-end justify-between">
                        <h2 className="font-serif text-2xl text-stone-900">Preparation kits</h2>
                    ...
                    {isError && (
                        <div className="rounded-2xl border border-red-200 bg-red-50 px-6 py-10 text-center">
                            <p className="text-sm text-red-700">Unable to load your preparation kits.</p>
                    ...
                    {!isLoading && !isError && kits.length === 0 && (
                        <div className="rounded-2xl border border-dashed border-stone-300 bg-white px-6 py-16 text-center">
                            <p className="font-serif text-xl text-stone-800">No preparation kits yet.</p>
                        ...
                        <div className="space-y-3">
                            {kits.map((kit, i) => (
                                <button
                                    ...
                                    className="group flex w-full items-center gap-5 rounded-2xl border border-stone-200 bg-white p-5 text-left transition-all hover:-translate-y-0.5 hover:border-stone-900 hover:shadow-[0_8px_24px_-12px_rgba(0,0,0,0.25)] sm:p-6"
                                >
                                    ...
                                    <span className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-stone-100 text-stone-500 transition-colors group-hover:bg-stone-950 group-hover:text-amber-400">
                                        →
                                    </span>
                                </button>
                            ))}
